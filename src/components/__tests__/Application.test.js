...
it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
  ...
  fireEvent.click(getByText(appointment, "Save"));
  expect(getByText(appointment, "Saving")).toBeInTheDocument();
  await waitForElement(() => getByText(appointment, "Mandy"));
  expect(getByAltText(appointment, "Edit"));
  const day = getAllByTestId(container, "day").find(day =>
    queryByText(day, "Monday")
  );
...
it("shows the save error when failing to save an appointment", async () => {
  ...
  expect(getByText(appointment, "There is a error while saving, pelse try again")).toBeInTheDocument();
...
it("shows the delete error when failing to delete an existing appointment", async () => {
  ...
  await waitForElement(() => getByText(appointment, "Error"));
  expect(getByText(appointment, "There is a error while deleting, pelse try again")).toBeInTheDocument();
  fireEvent.click(getByAltText(appointment, "Close"));
  expect(getByAltText(appointment, "Edit"));
})
